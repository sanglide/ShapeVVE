...
from ShapeVVE.PredictorModel.grad import TorchGradMixin
from ShapeVVE.PredictorModel.api import TorchClassMixinChannel, TorchPredictMixin
...
class MinEuclideanDistBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f'x.shape: {x.shape} shapelets.shape: {self.shapelets.shape}')
    ...
    def compute_shapelet_distances(self,input_tensor, shapelets):
        """
        Args:
            input_tensor:   [n_sample, dimension, window_num, window_length]
            shapelets:      [dimension, num_shapelets, window_length]
        Returns:
            distances:      [n_sample, num_shapelets]
        """

        # 2. 计算每个样本窗口与所有shapelets的距离
        # 使用广播机制自动对齐维度
        # input_tensor: [n_sample, dimension, window_num, window_length]
        # shapelets:    [num_shapelets, dimension, window_length]

        # print(f'shapelets: {shapelets.shape}, input_tensor: {input_tensor.shape}')
        input_tensor_flatten=(input_tensor.permute(0, 2, 1, 3).flatten(0, 1))
        input_tensor_flatten=input_tensor_flatten.permute(1,0,2)
        # print(f'{input_tensor_flatten.shape}')
        input_tensor_flatten = input_tensor_flatten.to(torch.float32)  # 确保输入是float32
        shapelets = shapelets.to(torch.float32)  # 确保shapelets也是float32


        distances = torch.cdist(
            input_tensor_flatten,  # [ dimension, n_sample*window_num, window_length]
            shapelets,  # [ dimension,num_shapelets, window_length]
            p=2  # 欧式距离
        )  # 结果形状: [dimension,n_sample*window_num, num_shapelets]

        # 1. 对 dimension 维度求平均 -> [n_sample * window_num, num_shapelets]
        mean_distance = distances.mean(dim=0)  # 沿dimension维度求平均
        reshaped_distance = mean_distance.view(input_tensor.size(0), input_tensor.size(2), -1)
        # -1自动推断为num_shapelets
        # 3. 对 window_num 维度取最小值 -> [n_sample, num_shapelets]
...
class ShapeletsDistanceLoss(nn.Module):
    ...
    def forward(self, x):
        ...
        k=x.shape[0] if self.k>x.shape[0] else self.k

        y_top, y_topi = torch.topk(x.clamp(1e-8), k, largest=False if self.dist_measure == 'euclidean' else True,
                                   sorted=False, dim=0)
...
class LearningShapelets(TorchClassMixinChannel, TorchPredictMixin, TorchGradMixin):
    """
    ...
    def fit(self,X, Y, epochs=1, batch_size=256, shuffle=False, drop_last=False,**kwargs):
        """
        Train the model.
        ...
        losses_dist = []
        losses_sim = []
        # progress_bar = tqdm(range(epochs), disable=False if self.verbose > 0 else True, mininterval=1)
    ...
    def transform(self, X):
        ...
        return shapelet_transform.squeeze().to(self.device).detach()
    ...
    def predict(self, X, batch_size=256):
        ...
        with torch.no_grad():
            for x in dl:
                y_hat = self.model(x[0])  # 前向传播
                result.append(y_hat)  # 收集原始输出（未Softmax）

        # 合并所有批次的预测结果
        result = torch.cat(result, dim=0)  # 形状为 [N, n_classes]
        result = torch.argmax(result, dim=1)  # 直接取argmax得到类别索引 [N]

        return result

    def get_shapelets(self):
        return self.model.get_shapelets().clone().to(self.device).detach()

    def get_weights_linear_layer(self):
        return (self.model.linear.weight.data.clone().to(self.device).detach(),
                self.model.linear.bias.data.clone().to(self.device).detach())