...
from ShapeVVE.DataLoader.register import Register
from ShapeVVE.DataLoader.util import CatDataset
...
class DataFetcher:
    ...
    def from_data_splits(
        ...
        y_train: Union[Dataset, np.ndarray],
        x_valid: Union[Dataset, np.ndarray],
        y_valid: Union[Dataset, np.ndarray],
        x_test: Union[Dataset, np.ndarray],
        y_test: Union[Dataset, np.ndarray],
        one_hot: bool,
        random_state: Optional[RandomState] = None,
    ):
    ...
    def datapoints(self):
        ...
        if not isinstance(self.x_train, Dataset) and not isinstance(
            ...
            x_trn = (x_trn.clone().detach()).view(-1, *self.covar_dim)
            x_val = (x_val.clone().detach()).view(-1, *self.covar_dim)
            x_test = (x_test.clone().detach()).view(-1, *self.covar_dim)

        y_trn =(self.y_train.clone().detach()).view(-1, *self.label_dim)
        y_val = (self.y_valid.clone().detach()).view(-1, *self.label_dim)
        y_test = (self.y_test.clone().detach()).view(-1, *self.label_dim)

        # print(f'[DEBUG] line 275 in fetcher.py  {x_trn.shape} {y_trn.shape}')
    ...
    @property
    def covar_dim(self) -> tuple[int, ...]:
        """Get the partially flattened dimension of covariates (keep batch and seq, flatten the rest).

        Returns
        -------
        tuple[int, ...]
            Always returns (seq_len, flattened_features),
            e.g., for input (30,4,65,2), returns (4, 130).
        """
        data = self.covar if hasattr(self, "covar") else self.x_train
        ...
        if isinstance(data, Dataset):
            ...
            if isinstance(sample, (str, float, int)):
                return (1,)  # Scalar case
            elif isinstance(sample, (np.ndarray, torch.Tensor)):
                if sample.ndim <= 2:
                    return (sample.shape[-1],)  # (seq_len,) or (flattened_features,)
                else:
                    # Keep dim[1] as seq_len, flatten the rest
                    return (sample.shape[1], np.prod(sample.shape[2:]).astype(int))
            else:
                return (1,)  # Unknown type
        else:
            # Handle numpy array or torch.Tensor
            if data.ndim <= 2:
                return (data.shape[-1],)  # (seq_len,) or (flattened_features,)
            else:
                # Keep dim[1] as seq_len, flatten the rest
                return (data.shape[1], int(np.prod(data.shape[2:])))

    def prepare_data(self, x):
        """Partially flatten input to (batch, seq, flattened_features).

        Args:
            x: Input tensor of shape (batch, seq, ...), where ... can be any dimensions.

        Returns:
            torch.Tensor of shape (batch, seq, prod(...)),
            e.g., (30,4,65,2) -> (30,4,130).
        """
        x_tensor = torch.as_tensor(x, dtype=torch.float32)

        if x_tensor.ndim <= 2:
            # If input is (batch, features), add a seq=1 dimension
            return x_tensor.unsqueeze(1)  # (batch, 1, features)
            ...
            # Flatten all dimensions after seq
            batch_size, seq_len = x_tensor.shape[0], x_tensor.shape[1]
            return x_tensor.view(batch_size, seq_len, -1)
            return x_tensor.view(batch_size, seq_len, -1)
    @property
