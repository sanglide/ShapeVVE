...
from ShapeletsVal.ShapeletsVE.metrics import accuracy, neg_mse
from ShapeletsVal.PredictorModel.api import Model
from ShapeletsVal.ShapeletsVE.util import ReprMixin
...
class DataEvaluator(ABC, ReprMixin):
    ...
    def input_fetcher(self, fetcher: DataFetcher):
        ...
        if isinstance(x_valid, (Dataset, Subset)):
            x_valid = torch.stack([x for x in x_valid])

        return self.input_data(x_train, y_train, x_valid, y_valid,x_test,y_test)
